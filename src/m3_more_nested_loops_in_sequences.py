...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 64
    answer = largest_number(([4, 8, 16], [32, 64], [3, 2, 1]))
    print('Expected and actual are:', expected, answer)
    ...
    for k in range(len(seq_seq)):
        for i in range(len(seq_seq[k])):
            if seq_seq[k][i] == None:
                ...
                if biggest_num == None:
                    biggest_num = seq_seq[k][i]
                else:
                    if seq_seq[k][i] > biggest_num:
                        biggest_num = seq_seq[k][i]
...
def run_test_largest_negative_number():
    ...
    # Test 1:
    expected = -3
    answer = largest_negative_number([[2, -3, -48],[8, 10, -22], [7]])
    print('Expected and actual are:', expected, answer)

    # Test 2:
    expected = -1
    answer = largest_negative_number([[5, -10000000], [3, -1, -8], [10000]])
    print('Expected and actual are:', expected, answer)

    # Test 3:
    expected = None
    answer = largest_negative_number([[10000, 10000000], [30000, 1, 800],
                                      [10000]])
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for i in range(len(seq_seq[k])):
            if seq_seq[k][i] < 0:
                if most_negative == 0:
                    most_negative = seq_seq[k][i]
                else:
                    if seq_seq[k][i] > most_negative:
                        most_negative = seq_seq[k][i]
    if most_negative == 0:
        return None
    else:
